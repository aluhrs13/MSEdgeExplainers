...
There is no need to add any special handler for the shortcut aside from having a URL (which is part of the general site architecture) to respond to the action. A user agent should create a [new](https://html.spec.whatwg.org/multipage/#creating-a-new-browsing-context) [top-level browsing context](https://html.spec.whatwg.org/multipage/#top-level-browsing-context) to respond to the shortcut activation.


The proposed [`ServiceWorker` Launch Event](https://github.com/WICG/sw-launch), while not tied to Shortcuts in any way, could also be useful for responding to shortcut actions:
